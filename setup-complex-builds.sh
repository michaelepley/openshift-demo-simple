...
# Using jenkins oauth authentication, delegating to openshift
...
# Using jenkins password authentication, u:admin p:password
oc get dc/jenkins || oc process openshift//jenkins-ephemeral -l app=${OPENSHIFT_APPLICATION_NAME},part=cicd JENKINS_PASSWORD=password OPENSHIFT_ENABLE_OAUTH=false | oc create -f - || { echo "FAILED: Could not create Jenkins CICD server" && exit 1; }
